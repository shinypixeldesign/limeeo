...
import { emitToSmartBillAction, syncSmartBillStatusAction } from '@/app/actions/smartbill'
...
export default function SmartBillButton({ invoiceId, smartbillNumber, invoiceStatus }: Props) {
  ...
  function handleSync() {
    setResult(null)
    startTransition(async () => {
      const res = await syncSmartBillStatusAction(invoiceId)
      setResult(res ?? null)
    })
  }

  // Dacă deja emisă — arată badge verde + buton sincronizare
  if (smartbillNumber) {
    return (
      <div className="flex flex-col items-end gap-1.5">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleSync}
            disabled={isPending}
            title="Sincronizează status din SmartBill"
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 transition disabled:opacity-60 disabled:cursor-not-allowed"
          >
            <svg
        ...
        {result?.error && <p className="text-xs text-red-600 font-medium">{result.error}</p>}
        {result?.message && <p className="text-xs text-emerald-600 font-medium">{result.message}</p>}
      </div>
    )
  }
