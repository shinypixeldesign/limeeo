...
import { deleteInvoiceAction } from '@/app/actions/financials'
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-sm text-slate-600">Ștergi <strong>{invoiceNumber}</strong>?</span>
        <form action={deleteInvoiceAction} className="inline">
          <input type="hidden" name="id" value={invoiceId} />
          <button type="submit" className="rounded-lg bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-700 transition">
            Da, șterge
          </button>
        </form>
        <button
