...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: runningTimers } = await supabase
      .from('time_entries')
      .select('id, user_id, started_at, description, timer_reminder_sent')
      .is('ended_at', null)
    ...
    for (const timer of runningTimers ?? []) {
      try {
        ...
        const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'https://app.limeeo.com'
          ...
          await resend.emails.send({
            ...
            html: `
              <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#fff;border-radius:16px;">
                <div style="background:#acff55;width:48px;height:48px;border-radius:12px;display:inline-flex;align-items:center;justify-content:center;margin-bottom:20px;font-size:24px;">⏱</div>
                ...
                <a href="${appUrl}/time" style="display:inline-block;background:#acff55;color:#000;font-weight:700;padding:12px 24px;border-radius:999px;text-decoration:none;font-size:14px;">
                  Mergi la Pontaj →
                </a>
